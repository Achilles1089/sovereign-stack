...
class LlamaEngine: ObservableObject {
    ...
    func complete(
        ...
        guard let model = model, let ctx = ctx, let sampler = sampler else {
        ...
        let nTokens = llama_tokenize(model, promptCStr, Int32(promptCStr.count - 1), &tokens, nPromptTokensMax, true, true)
        ...
        for (i, token) in tokens.enumerated() {
            ...
            batch.n_seq_id[i] = 1
            batch.seq_id[i]![0] = 0
            batch.logits[i] = (i == tokens.count - 1) ? 1 : 0 // Only compute logits for last token
        }
        ...
        while nGenerated < maxTokens && !isCancelled() {
            ...
            let nChars = llama_token_to_piece(model, newToken, &buf, 256, 0, true)
            if nChars > 0 {
